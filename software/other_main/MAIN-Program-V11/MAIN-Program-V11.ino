...
#define TIME_BETWEEN_SCREEN 500 // time between each window ms         
#define DELAY_ANTI_REBOUND 10000 // Push button us
...
void loop() {
char X=X;
  TestPushButtonScreen(); BetweenScreen(X);
    
  DisplayXaxisMPUScreen(); BetweenScreen(X);

  ScreenKPsetting(); BetweenScreen(X);
  
  ScreenKIsetting(); BetweenScreen(X);

  ScreenKDsetting(); BetweenScreen(X);
...
void Regulation(void){
  ...
  commande += OFFSET_DEMARAGE_MOTEUR;
    ...
    commande = OFFSET_DEMARAGE_MOTEUR - commande;
    if(commande > 255){commande = 255;}
...
void BetweenScreen(const char letter){
