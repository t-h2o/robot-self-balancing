...
// Régulation
float kP = 5.0, kI = 0, kD = 0.0;
float lastError = 0;

// Moyenne glissante
byte numeroTableau = 0;
float tableauAngle[4] = {0,0,0,0};
...
void loop_mpu_6050(void){
  ...
  while((micros() - loop_timer) < 4000){} // can't go up 250 Hz
...
void Regulation(void){
  ...
  if(numeroTableau++ > 4){numeroTableau = 0;}
  ...
  angleGlissant = angleGlissant / 4; 
  */
  float error = 0 - angle_pitch_output;
  float deltaError = angle_pitch_output - lastError;
  ...
  if (displaycount == 255) {
    ...
    u8g2.setCursor(0, 13); // 25 21
    u8g2.print("axis X : "); u8g2.println(angleGlissant);
