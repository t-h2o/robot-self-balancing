...
uint32_t gyro_x_cal, gyro_y_cal, gyro_z_cal;
boolean set_gyro_angles;

uint32_t acc_x, acc_y, acc_z, acc_total_vector;
...
float lastError = 0;
byte displaycount = 0;// Display counter  

// Moyenne glissante
byte numeroTableau = 0;
float tableauAngle[8] = {0,0,0,0 ,0,0,0,0};
float angleGlissant=0;
...
// Affiche un écran d'acceuil 
  BetweenScreen();
  HomeScreen();

// Affiche le test des boutons  
  BetweenScreen();
...
void ScreenKPsetting(void){
  ...
  do{
    ...
    u8g2.setCursor(0, 30); u8g2.print("inc. :"); u8g2.print(angleGlissant);
...
void TestPushButtonScreen(void){
  /* Affiche comme premier bit
   * l'état du bouton et comme 2e
   * le set-reset du bouton
   * ça permet de tester 
...
void DisplayXaxisMPUScreen(){
  ...
  do{ 
    ...
    u8g2.setCursor(0, 30); u8g2.print("axis X : "); u8g2.println(angleGlissant);
    RefreshScreen();
...
void loop_mpu_6050(void){
  ...
  // fréquence = 1000000 us / (micros us - ancienneLoop us)
  frequenceRegulation = 1000000 / (micros() - loop_timer);
  
  //Gyro angle calculations . Note 0.0000611 = 1 / (250Hz x 65.5)
  float gyroAngleCalculation = 1 / (frequenceRegulation * 65.5);
  ...
  // Print to Serial Monitor   
    ...
    // moyenne glissante
  tableauAngle[numeroTableau] = angle_pitch_output;
  
  if(numeroTableau++ > 8){numeroTableau = 0;}
  
  
  
  for(byte i=0; i<8; i++){
    angleGlissant += tableauAngle[i];
  }
  angleGlissant = angleGlissant / 8;
...
void Regulation(void){
  uint32_t tRegulationStart=micros();
  float error = 0 - angleGlissant;
  float deltaError = lastError - error;
  float commande = error * kP + deltaError * kD + 0.0;
  ...
  u8g2.setCursor(0, 13); u8g2.print("axis X : "); u8g2.println(angleGlissant);
  u8g2.setCursor(0, 30); u8g2.print("fréq :"); u8g2.println(frequenceRegulation);
  ...
  lastError = error;
  frequenceRegulation = 1000000 / (micros()-tRegulationStart);
}
